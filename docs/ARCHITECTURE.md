...
| ドメイン | 書き手 | 契機 |
|---|---|---|
| お知らせ本体・翻訳 | support | 運用者による登録・更新（管理 UI は提供していない。§ 管理 UI と問い合わせフォームを提供していない理由） |
| 問い合わせ受付 | なし | 受け付けていない（同上） |
| 問い合わせステータス・対応メモ | なし | 更新経路を提供していない（同上） |
...
## 管理 UI と問い合わせフォームを提供していない理由

管理 UI と問い合わせ受付フォームのサーバは起動しない。Cloud Run は 1 リビジョンにつき 1 ポートしか公開せず、管理 UI の認証を肩代わりしていた IAP も公開ロードバランサの廃止で無くなったため、到達させるには構成と認証を作り直す必要がある。お知らせの登録・更新は当面運用者の手作業とし、問い合わせは外部フォームで受ける。管理機能と問い合わせは将来別サービスとして用意する。

以降の管理 UI・問い合わせ受付に関する記述は、その別サービスに引き継ぐ設計として残す。

### 管理 UI と外部フォームを内部 API と同じポートに置かない

- 外部公開面と内部 API を同一ポートで束ねると、単一の設定ミスで内部 API が外部到達可能になる
- IAP は Ingress / LB 単位で設定するため、`/admin/*` だけに IAP 認証を強制することはできない。`/admin/*` を公開ポートと混在させると、誤って内部向けの経路に露出する構造的リスクが生まれる
- ポートごとに経路を分けることで、「管理 UI は IAP 経由のみ」「問い合わせフォームは外部からのみ」「gateway は内部からのみ」という信頼境界をインフラ定義のレベルで固定できる
- CORS の適用範囲もポート単位に閉じ込められる（問い合わせフォームのポートのみ Origin 許可、他ポートは CORS 無効）

### CORS

問い合わせ受付の `/api/v1/inquiries` は問い合わせフォームのオリジンからのみ受ける。許可オリジンは env で指定。内部 API・管理 UI 側は CORS を設定しない。
...
- 大量スパムが観測されたタイミングで問い合わせ受付の前段に Cloud Armor / reCAPTCHA 等を入れる前提とする
...
- 問い合わせ受付を提供していないため、Slack bot token / channel ID と SendGrid API key は起動時に読まない
- ローカル開発では `make run` が env を自動注入する

### 外部依存とヘルスチェック

`/health` は DB 接続のみチェックする。問い合わせ受付を持つ側でも Slack / SendGrid の外部連携は健康チェック対象に含めない（外部サービスのメンテで unhealthy 扱いになると、API そのものは生きているのに配信経路から切り離されるため）。Slack / SendGrid の不調は副作用失敗時の ERROR ログと運用監視（Cloud Logging アラート）で検知する。

### 管理 UI の可用性

管理 UI は提供していないため、SLO の対象は内部 API だけになる。将来別サービスとして用意する管理 UI は運用者の業務時間中のみ使われる内部ツールであり、24/7 SLO は設けない。
